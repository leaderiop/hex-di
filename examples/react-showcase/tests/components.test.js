...
import { describe, it, expect, vi, screen, fireEvent, waitFor, render, TestGraphBuilder, createMockAdapter, } from "./setup.js";
import { createContainer } from "@hex-di/runtime";
import { appGraph } from "../src/di/graph.js";
import { UserSessionPort, MessageStorePort, ChatServicePort, NotificationServicePort, LoggerPort, ConfigPort, } from "../src/di/ports.js";
 ...
 */
function renderWithAppContainer(element, graph) {
    const diContainer = createContainer(graph);
    const { container } = render(_jsx(ContainerProvider, { container: diContainer, children: element }));
...
describe("ChatRoom", () => {
    it("renders with AutoScopeProvider and displays user switch buttons", () => {
        ...
        const testGraph = createTestGraph()
            ...
            .build();
        renderWithAppContainer(_jsx(ChatRoom, {}), testGraph);
...
describe("MessageList", () => {
    it("displays messages from the store", () => {
        ...
        const testGraph = createTestGraph()
            .override(mockMessageStoreAdapter)
            .build();
        renderWithAppContainer(_jsx(MessageList, {}), testGraph);
...
describe("MessageInput", () => {
    it("sends message via ChatService when send button is clicked", async () => {
        ...
        renderWithAppContainer(_jsx(MessageInput, {}), testGraph);
        // Type a message
...
describe("UserInfo", () => {
    it("displays current user name and avatar", () => {
        ...
        renderWithAppContainer(_jsx(UserInfo, {}), testGraph);
        expect(screen.getByText("Alice")).toBeInTheDocument();
...
describe("NotificationButton", () => {
    it("shows new instance ID on each click", async () => {
        ...
        const testGraph = createTestGraph()
            .override(mockNotificationService)
            .build();
        renderWithAppContainer(_jsx(NotificationButton, {}), testGraph);
